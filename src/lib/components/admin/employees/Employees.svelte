<script lang="ts">
    ...
    import {getEmployees, type Paged} from "$lib/api/employees";
    
    $:employees = createQuery<Paged<EmployeeListItemDto>>({
        queryKey: ['employees', page],
        queryFn: async() =>  await getEmployees(page),
    });

    let page = 1;
    function prev() { if (meta.hasPrev) page -= 1; }
    function next() { if (meta.hasNext) page += 1; }
    function go(n: number|string) { if (n as number >= 1 && n as number <= meta.totalPages && n !== page) page = n as number; }

    $: meta = $employees.data?.meta ?? { page, perPage: 10, total: 0, totalPages: 1, hasPrev: false, hasNext: false };

    // «Showing X–Y of Z»
    $: from = meta.total ? (meta.page - 1) * meta.perPage + 1 : 0;
    $: to   = meta.total ? Math.min(meta.page * meta.perPage, meta.total) : 0;
    ...
    function buildPages(current: number, total: number) {
        ...
        const c = Math.min(Math.max(1, current || 1), t);
        const cand = new Set<number>([1, t, c - 1, c, c + 1, 2, t - 1]);
        const nums = Array.from(cand).filter(n => n >= 1 && n <= t).sort((a,b)=>a-b);
    ...
    $: pages = buildPages(meta.page, meta.totalPages);
            ...
            <div class="overflow-x-auto">
                    ...
                    {#if $employees.isSuccess}
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <EmployeeTableHead/>
                            <tbody>
                                {#each $employees.data.data as employee}
